...
def test_clear_combatants(sample_battle, sample_meal_1): 
    """Check to see if clear the combatants array"""
    sample_battle = BattleModel()
    sample_battle.combatants.append(sample_meal_1)
...
def test_empty_clear_combatants(battle_model, caplog):
    ...
    assert "Clearing the combatants list." in caplog.text, "Expected warning message when clearing an empty playlist"
